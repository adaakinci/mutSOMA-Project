makeVCFpedigreeTEMPv2<-function(genome.size, input.dir, tree13, tree14)
{
  ...
  #vcf13 <- vcf[!duplicated(paste(vcf[, "CHROM"], vcf[, "POS"], sep = ":")), ]
  vcf13_sample_names <- colnames(vcf13)[6:ncol(vcf13)]
  vcf13_ids <- sub(".*13\\.(\\d+)$", "\\1", vcf13_sample_names)
  colnames(vcf13)[6:ncol(vcf13)] <- make.unique(paste0("tree13.", vcf13_ids))
  print(colnames(vcf13)[6:ncol(vcf13)])
  ...
  #vcf14 <- vcf[!duplicated(paste(vcf[, "CHROM"], vcf[, "POS"], sep = ":")), ]
  
  vcf14_sample_names <- colnames(vcf14)[6:ncol(vcf14)]
  vcf14_ids <- sub(".*14\\.(\\d+)$", "\\1", vcf14_sample_names)
  colnames(vcf14)[6:ncol(vcf14)] <- make.unique(paste0("tree14.", vcf14_ids))
  print(colnames(vcf14)[6:ncol(vcf14)])
  ...
  index.this<-c(1,2,3,4,5,9,8,7,6, 10, 11, 12, 13)
  colnames(vcf)<-colnames(vcf)[index.this]
  ...
  {
    take1<-div[which(paste(div[,1], div[,2]) == paste(pedigree.new[a,1], pedigree.new[a,2])),]
    if (nrow(take1)== 0){take1<-div[which(paste(div[,2], div[,1]) == paste(pedigree.new[a,1], pedigree.new[a,2])),]}
    div.collect[a]<-take1[,3]
  }
  print(div.collect[1])
